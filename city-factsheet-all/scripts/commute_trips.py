...
def legs_start_work():
    cols = ['leg id','purpose',  'start node', 'end node','duration (congested)']
    legs = pd.read_csv(raw_legs_path, sep = '\t', usecols = cols)

    logger.info("==== Loading the clipped city links to get all nodes in a city ===")
    ...
    legs_start.to_csv(os.path.join(write_directory, f"legs_start_{city_name}.csv"),  index = False)
    legs_work_start.to_csv(os.path.join(write_directory, f"legs_start_work_{city_name}.csv"),  index = False)
...
def mean_commute_tt(trips_start_work_city_path):
    ...
    mean_worktrip_minutes =  np.round(legs_work_start['congested_ttmin'].mean(), decimals = 1)
